        ...
        //user gallery array will be updated with a new item
            ...
            Feed.find({}, (err, foundFeed) => {
                if(err) {
                    ...
                    const link = `/galleries/${addedArtPost._id}`;
                    const text = 'posted a new gallery item';
                    const image = addedArtPost.image;
                    const title = addedArtPost.name;
                    const content = '';
                    let feedPost = new Feed({author: author, text: text, link: link, image: image, title: title, content: content})
                    feedPost.save();
                    console.log('feed object', feedPost)
    ...
    Gallery.findById(id, (err, gallery) => {
        ...
        Feed.find({}, (err, foundFeed) => {
            if(err) {
                ...
                const link = `/galleries/${gallery._id}`;
                const title = '';
                const content = '';
                console.log('this is the author image', author.image)
                let feedPost = new Feed({author: author, link: link, image: image, title: title, content: content, text: `${author.username} liked ${gallery.author.username} artwork.`})
                feedPost.save();
                console.log('feed object', feedPost)
