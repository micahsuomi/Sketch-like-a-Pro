        ...
        if(err) {
            ...
            Comment.create(req.body.comment, (err, comment) => {
                if(err) {
                    ...
                    console.log('this is the comment text', req.body.comment)
                    comment.save();
                    ...
                    Feed.find({}, (err, foundFeed) => {
                        if(err) {
                            ...
                            const link = `/galleries/${id}`;
                            const image = '';
                            const title = '';
                            const content = '';
                            let feedPost = new Feed({author: author, link: link, image: image, title: title, content: content, text: `commented on ${gallery.author.username} artwork: ${req.body.comment.text}`})
                            feedPost.save();
