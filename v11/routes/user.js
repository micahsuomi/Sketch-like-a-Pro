...
router.get('/', (req, res) => {
    User.find().populate('gallery').exec((err, users) => {
        if(err) {
            req.flash('Something went wrong');
            res.redirect('back');
        } 
        // Gallery.find().where('author.id').equals(req.user._id).exec((err, gallery) => {
        //     if(err) {
        //         req.flash('Something went wrong');
        //         res.redirect('back');
        //     }
...
router.get('/:id', middleware.isLoggedIn, (req, res) => {
    ...
    User.findOne({_id: id}).populate("gallery").populate("posts").populate("comments").exec((err, user) => {
        ...
        Feed.find().where('author.id').equals(user._id).exec((err, feeds) => {
            if(err) {
                req.flash('Something went wrong')
                res.redirect('back')
            }
            ...
            if(total > 0) {
                const saveAll = () => {
                    ...
                    saveAll();  
                        ...
                        feeds.reverse();
                        posts.reverse();
                        res.render('users/show', {user, feeds, gallery, posts})
...
//SHOW route for feeds for a single user
router.get('/:id/feeds', (req, res) => {
    Feed.find().where('author.id').equals(req.user._id).exec((err, foundFeeds) => {

        if(err) {
            req.flash('Something went wrong');
            res.redirect('back');
        } else {
            console.log(req.user._id, foundFeeds)
            res.render('feeds/show', {foundFeeds})
        }
    })
})
